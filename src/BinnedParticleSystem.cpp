...
void BinnedParticleSystem::setup(int width, int height, int k) {
	...
	this->height = height;
	this->k = k;
	binSize = 1 << k; // bitwise operator, based on comment in ofApp.h and https://stackoverflow.com/questions/18215681/what-is-1-0 it is equivalent of saying k to the power of 2 (or k?)
    cout << binSize << endl;
    cout << 1*pow(2,k) << endl;
